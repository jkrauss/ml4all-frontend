		...
		<form on:submit|preventDefault={sendSignup}>
			<Textfield bind:value={name} label="Name" type="name">
				<HelperText slot="helper"
					>Wie dürfen wir Sie ansprechen?</HelperText
				>
			</Textfield>
			<Textfield bind:value={email} label="E-Mail" type="email" required>
				<HelperText slot="helper">Gültige E-Mail-Adresse</HelperText>
			</Textfield>
			<Textfield bind:value={phone} label="Telefon" type="phone" required>
				<HelperText slot="helper"
					>Benötigt z.B. wenn Sie ihr Passwort nicht mehr wissen</HelperText
				>
			</Textfield>
			<Textfield
				bind:value={password}
			...
			>
				<Icon
					...
					slot="trailingIcon"
					on:click={() => togglePwd()}>{pwdIcon}</Icon
				>
				<HelperText slot="helper"
					>Mind.12 Zeichen, Buchstaben, Zahlen, Sonderzeichen</HelperText
				>
			</Textfield>
			<Textfield bind:value={location} label="Standort" type="text">
				<HelperText slot="helper"
					>Wo befindet sich Ihr (erstes) Geschäft?</HelperText
				>
			</Textfield>
			<Textfield bind:value={register} label="Kassensystem" type="text">
				<HelperText slot="helper"
					>Welches Kassensystem verwenden Sie?</HelperText
				>
			</Textfield>
			<FormField>
