<script>
	...
	let data = [], //raw json data from source
		...
		blacklist = ["comment", "forecast"], // colum blacklist
		labels = [
		...
		styling = {
			...
			even: { bg: "var(--mdc-theme-error)", text: "" }, //defines colors for even rows
			odd: { bg: "green", text: "var(--mdc-theme-background)" }, // var(--VARNAME) can be used to use the vars from foodsight.css
		}; // horible styling xD
